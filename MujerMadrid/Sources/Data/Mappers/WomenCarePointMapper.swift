...
final class WomenCarePointMapper: @unchecked Sendable, WomenCarePointMapperContract {
    ...
    public func map(_ input: WomenCarePointDataEntity) throws -> WomenCarePointModel {
        ...
        let data = dataInfo.compactMap { entity in
            ...
            let location = WomenCarePointModel.LocationModel(latitude: entity?.location?.latitude,
                                                             longitude: entity?.location?.longitude)
            let address = WomenCarePointModel.AddressModel(locality: entity?.address?.locality,
            ...
            let organization = WomenCarePointModel.OrganizationModel(accesibility: entity?.organization?.accesibility,
            ...
            let recurrence = WomenCarePointModel.RecurrenceModel(interval: entity?.recurrence?.interval,
                                                                 days: entity?.recurrence?.days,
                                                                 frequency: entity?.recurrence?.frequency)
            let type = entity?.type
            let url = entity?.url
            
            return WomenCarePointModel.EventModel(id: id,
                                                  uid: uid,
                                                  dtstart: dtstart,
                                                  dtend: dtend,
                                                  title: title,
